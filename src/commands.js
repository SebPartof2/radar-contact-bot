...
export async function handleInteraction(interaction) {
  ...
  if (sub === 'list') {
    ...
    const lines = watches.map((w) => {
      const target = w.kind === 'cid' ? `CID \`${w.value}\`` : `\`${w.value}_*\``;
      return `• ${target}${w.label ? ` — ${w.label}` : ''} (added by <@${w.added_by}>)`;
    });
  ...
  switch (sub) {
    case 'watch-cid': {
      ...
      const added = db.addWatch(guildId, 'cid', cid, label, interaction.user.id);
      return reply(
        added ? `Now monitoring CID \`${cid}\`.` : `CID \`${cid}\` is already monitored.`,
      );
    }
    ...
    case 'watch-prefix': {
      ...
      const added = db.addWatch(guildId, 'prefix', prefix, label, interaction.user.id);
      return reply(
        added
